...
{
	if (!(((*all).line[(*all).ii + 2] == ' ' && (((*all).line[(*all).ii]
						== 'S' && (*all).line[(*all).ii + 1] == 'O')
					|| ((*all).line[(*all).ii] == 'N'
						&& (*all).line[(*all).ii + 1] == 'O')
					|| ((*all).line[(*all).ii] == 'W'
						&& (*all).line[(*all).ii + 1] == 'E')
...
void	paste_no_so_we_ea_s(t_mlx *all, int i)
{
	...
	if ((*all).line[(*all).ii] == 'S' && (*all).line[(*all).ii + 1] == ' ')
...
void	paste_r_f_c_part1(t_mlx *all, int *rfc, char c, int iter)
{
	char	comma_space;
	int		i;
